...
function add_user(){
    ...
    $is_super_admin = (isset($_POST['is_super_admin']) && $_POST['is_super_admin'] == "true") ? 1 : 0;

    if(!$f_name OR !$l_name OR !$username OR !$password) {
        echo json_encode(["success"=>false, 'errMsg'=>"Sorry, there was an error with required user data, please fill first name, last name, username and password correctly."]);
        return false;
    }
...
function update_game_data(){
    ...
    if(isset($_POST['quantity']) AND $_POST['quantity']) $game_data['quantity'] = $_POST['quantity']?:'';
...
function start_update_game($game_data=[]){
    ...
    if(isset($game_data['quantity'])) $sql_values.=($sql_values?", ":' ')."quantity=:quantity";
...
function get_all_users(){
    ...
    $sql = "SELECT id, f_name, l_name, username, created_at FROM users ORDER BY id";
    $stmt = $pdo->query($sql);
    $users = $stmt->fetchAll();
...
function delete_user(){
    ...
    if(!$count->rowCount()){
        echo json_encode(["success"=>false, 'errMsg'=>"Sorry, user not found."]);
        return false;
    }

    echo json_encode(["success"=>true, 'data'=>"User have been deleted successfully."]);
    return true;
}
