...
function SiteConfig({ user, games, systemPreferences, history }) {
  if (!user || (!parseInt(user.is_admin) && !parseInt(user.is_super_admin)))
        ...
        <Form>
          ...
          <Form.Group className="mb-3">
            <Form.Control
              ...
              onChange={(e) =>
                setNewGameData({ ...newGameData, quantity: e.target.value })
              }
            ></Form.Control>
          </Form.Group>
  ...
  const handleDeleteUserButton = (user_id = 0) => {
    setIsLoading(true);

    let formData = new FormData();
    formData.append("user_id", user_id);

    fetch(`http://${process.env.REACT_APP_IP_ADDRESS}?function=delete_user`, {
      method: "POST",
      body: formData,
    })
      .then((result) => result.json())
        ...
        CarouselCards.push(
          <Carousel.Item key={uuidv4()}>
            <div style={{ ...style }}>
              ...
              <Button
                variant="danger"
                onClick={() => handleDeleteUserButton(user_id)}
              >
                Delete User
              </Button>
              {isLoading && <Loading />}
      ...
      <div style={{ padding: "20px" }}>
        <Form>
          ...
          <Form.Group className="mb-3">
            <Form.Control
              type="number"
              placeholder={editGame.quantity}
              onChange={(e) =>
                setGameData({ ...gameData, quantity: e.target.value })
              }
            />
          </Form.Group>
